"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require('@angular/core');
var card_1 = require('@angular2-material/card/card');
var list_mock_1 = require('../mocks/list_mock');
var categories_mock_1 = require('../mocks/categories_mock');
var pipes_1 = require('../shared/pipes');
var pipes_2 = require('../shared/pipes');
var ListComponent = (function () {
    function ListComponent() {
    }
    ListComponent.prototype.ngOnInit = function () {
        this.list = list_mock_1.LIST;
        this.categories = categories_mock_1.CATEGORIES;
    };
    ListComponent = __decorate([
        core_1.Component({
            selector: 'list',
            templateUrl: './app/list/list.template.html',
            styleUrls: ['./app/list/list.css'],
            directives: [card_1.MD_CARD_DIRECTIVES],
            pipes: [pipes_1.ListCategoryPipe, pipes_2.ListImageCategoryPipe]
        }), 
        __metadata('design:paramtypes', [])
    ], ListComponent);
    return ListComponent;
}());
exports.ListComponent = ListComponent;
//# sourceMappingURL=list.component.js.map